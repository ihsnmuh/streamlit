...
# Inisialisasi session state untuk menyimpan catatan
...
# Action untuk menmbah catatan baru
if st.button("Tambah"):
...
st.write("---")
st.subheader("Daftar Catatan Anda:")
...
if not tasks :
    ...
    with col1:
        ...
        for idx, task in enumerate(tasks):
            if task.status == "todo":
                with st.expander(f"📋 {str(task.title).capitalize()} ({task.status})"):
                    st.write(task.description)

                    new_status = st.selectbox(
                        "Ubah Status:", status_option, 
                    ...
                    delete_btn = st.button("Hapus", key=f"delete_{idx}")
                    if delete_btn:
